//@ts-ignore
const square = (aNum: number, bNum: number):number => aNum * bNum;
console.log(square(5, 10));


const roundSquare = (radius: number): number => Math.PI * radius * radius;
console.log(roundSquare (6));


const squareCilinder = (radius: number, height: number): number => 2 * Math.PI * radius * height;
console.log(squareCilinder ( 10, 40) );


const foobar = (array: any[]) => {
...
const foobarText = (text: string): void => {
...
const foobarList = (text: string): void =>{
    document.write(`
                  <ul>
                    <li>${text}</li>
                    <li>${text}</li>
...
type Primitiv = string | number | boolean;
const foobarPrim = (arrayOfPrimitivs: Primitiv[]): void => {
    document.write(`<ul>`)
...
const foobarusers = (users: User49[]) =>{
...
const arrayMinValue = (numbers: number[]) => {
    let min = numbers[0];
...
const swap = (arr: number[], i1: number, i2: number): number[] | string | number => {

...
const coursesWithId: CourseWithId[] = coursesAndDurationArray.map(
    (course, index) => ({
        ...course,
        id: index + 1
    })
...
function cloner<T extends Record<string, any>>(obj: T): T {
    ...
    const functions: { key: string; functionClone: Function }[] = [];
